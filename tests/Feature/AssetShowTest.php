...
test('存在しない銘柄 ID は 404', function () {
    $user = User::factory()->create();

    $this->actingAs($user)
        ->get(route('assets.show', 999_999_999))
        ->assertNotFound();
});

test('数値 id で銘柄詳細が表示される', function () {
    $user = User::factory()->create();
    $btc = Asset::factory()->create(['symbol' => 'BTC']);

    $this->actingAs($user)
        ->get(route('assets.show', $btc->id))
        ->assertOk()
...
test('range クエリで価格履歴の範囲が絞られる', function () {
    ...
    $this->actingAs($user)
        ->get(route('assets.show', $btc->id).'?range=7d')
        ->assertOk()
        ->assertInertia(fn ($page) => $page
            ->where('range', '7d')
            ->has('prices', 1)
    ...
    $this->actingAs($user)
        ->get(route('assets.show', $btc->id).'?range=all')
        ->assertOk()
        ->assertInertia(fn ($page) => $page
            ->has('prices', 2)
        );
